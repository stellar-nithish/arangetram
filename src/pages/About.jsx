...
const About = () => {
  ...
  return (
    <main id="about-page" className="min-h-screen bg-accent">
      ...
      <section
        id="about-hero"
        className="relative bg-dark overflow-hidden pt-28 pb-20 px-4"
      >
        ...
        <div className="absolute -bottom-16 -right-16 w-72 h-72 rounded-full bg-secondary/10 blur-3xl pointer-events-none" />
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12">
          ...
          <motion.div
            ...
            custom={0}
          >
            <span className="inline-block text-secondary uppercase tracking-[0.3em] text-xs font-semibold mb-4">
              The Dancer Behind the Art
            </span>
            <h1 className="font-serif text-5xl md:text-6xl font-bold text-accent leading-tight mb-6">
              {about.title}
            </h1>
          ...
          >
            <div className="relative">
              {/* Gold shadow offset */}
              <div className="absolute inset-0 translate-x-4 translate-y-4 rounded-2xl bg-secondary/40" />
              <img
                src={images.about}
                alt="Sanjana Diddige"
                loading="lazy"
              ...
              />
              {/* Maroon glow ring */}
              <div className="absolute -inset-1 rounded-2xl ring-1 ring-secondary/30 z-20 pointer-events-none" />
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section id="about-timeline" className="py-24 px-4 bg-gradient-to-b from-accent/50 to-white overflow-x-hidden" ref={timelineRef}>
        <div className="max-w-5xl mx-auto">
          ...
          <div className="relative mt-16 md:mt-24">
            ...
            <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-[4px] bg-primary/10 -translate-x-1/2 rounded-full overflow-hidden">
               <motion.div 
            ...
            {/* Line for mobile */}
            <div className="md:hidden absolute left-8 top-0 bottom-0 w-[4px] bg-primary/10 -translate-x-1/2 rounded-full overflow-hidden">
               <motion.div 
                 className="absolute top-0 w-full bg-primary"
                 style={{ height: lineHeight }}
               />
              ...
              {TIMELINE_DATA.map((milestone, idx) => {
                ...
                return (
                  <div key={idx} className="relative flex flex-col md:flex-row justify-between items-start md:items-center w-full group">
                    ...
                    {/* Timeline Node (Year) */}
                    <div className="z-10 flex items-center justify-center bg-primary text-secondary font-bold text-sm md:text-lg rounded-full w-14 h-14 md:w-20 md:h-20 shadow-lg border-4 border-white shrink-0 absolute left-8 md:left-1/2 -translate-x-1/2 top-0 md:top-1/2 md:-translate-y-1/2 group-hover:scale-110 transition-transform duration-300 md:order-2">
                      {milestone.year}
                    </div>

                    {/* Mobile connecting line for Content Box */}
                    <div className="md:hidden absolute left-8 top-7 w-10 h-[2px] bg-primary/20 z-0" />
                    ...
                    <motion.div 
                      ...
                      custom={idx * 0.15}
                    >
                      <div className="bg-accent rounded-2xl p-6 md:p-8 shadow-sm border border-primary/10 hover:shadow-md transition-all relative">
                        {/* Connecting line to node (Desktop) */}
                        <div className={`hidden md:block absolute top-1/2 -translate-y-1/2 w-8 h-[2px] bg-primary/20 ${isEven ? '-right-8' : '-left-8'}`} />
                        <ul className="space-y-3">
                          {milestone.items.map((item, i) => (
                            <li key={i} className="flex items-start gap-3 text-left">
                              <span className="text-primary mt-[2px] text-lg leading-none shrink-0">•</span>
                              <span className="text-gray-700 leading-snug">{item}</span>
                            </li>
                          ))}
                        </ul>
                    ...
                    >
                      ...
                      <div className={`hidden md:block absolute top-1/2 -translate-y-1/2 w-8 h-[2px] bg-primary/20 ${isEven ? '-left-8' : '-right-8'}`} />
                      
                      <div className="relative w-full h-48 md:h-64 rounded-2xl overflow-hidden shadow-sm border-2 border-primary/10 hover:shadow-md transition-shadow group-hover:border-primary/30 duration-300">
                        <img 
                          src={milestone.image} 
      ...
      {/* ══════════════════════════════════
          JOURNEY HIGHLIGHTS
      ══════════════════════════════════ */}
      <section id="about-highlights" className="relative py-24 px-4 bg-dark overflow-hidden">
        {/* Subtle decorative glow for dark background */}
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-primary/20 rounded-full blur-[100px] pointer-events-none translate-x-1/2 -translate-y-1/2" />
